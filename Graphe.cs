...
namespace ProbSciANA
{
    public class Graphe
    {
        ...
        public HashSet<Noeud> ParcoursLargeur(int sommetDepart)
        {
        ...
        public HashSet<Noeud> DFSIteratif(int sommetDepart)
        {
            Dictionary<int, int> couleurs = new Dictionary<int, int>();
            ...
            while (pile.Count > 0)
            {
                ...
                {
                    if (couleurs[voisin] == 0)   // Cherche un voisin blanc
                    {
                        pile.Push(voisin);
                        couleurs[voisin] = 1; // jaune
...
        }
        ...
        {
            Console.Write("Parcours en Largeur (BFS):  ");
            foreach (Noeud sommet in ParcoursLargeur(sommetDepart))
            {
        ...
        {
            Console.Write("Parcours en Profondeur (DFS): ");
            DFSIteratif(sommetDepart);
            foreach (Noeud sommet in DFSIteratif(sommetDepart))
            {
                Console.Write(sommet.toString() + " ");
            }
        }
